...
#Segundo, como el output de readLines en cada caso fue un vector de caracteres de una sola columna, se obtuvo en su lugar cada matriz de conteo como un data.frame con read.delim(), puesto que esta función separa en columnas el vector por tabulación (\t)
...
#La base de datos de Ensembl (de las 4 que se mostraban como output de listMarts()) a la que se accedió a través de biomaRt fue a la de "ENSEMBL_MART_ENSEMBL", ya que era la que contenía la información de los genes
...
#Se obtuvo, por un lado, un archivo que solo tuviera los genes regulados al alza, y por otro lado, un archivo que solo tuviera los genes regulados a la baja, partiendo del archivo All_gene_lists_GMT (generado por iDEP96) en ambos casos
